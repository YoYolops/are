...
O máximo de correlação linear observada envolvendo a variável de interesse `Vendas`, foi de 46,2% de correlação com a variável `Estoque Médio`, indicando correlação
...
O gráfico também permite ver indícios que as variáveis apresentam distribuição normal, o que será verificado à frente quando observarmos se são válidas as suposições do MRLM.
...
Considerando nossa suspeita inicial de multicolinearidade enter as variáveis Marketing_Total e Publicidade, é possível que as mesmas tenham efeito detrimental sobre a determinação do modelo, sendo observável mais um indício de multicolinearidade que são seus sinais opostos. Assim, diagnosticamos a existência de multicolinearidade e, a seguir, selecionaremos uma delas para ser removida para verificar o impacto.

## Ajuste de modelo removendo a variáveis

```{r}
m2 <- update(m1, ~ . -Marketing_Total)
...
Considerando que o p-valor obtido (0.6489) é superior ao nível de significância determinado ao início do relatório, determinamos que a inclusão da variável `Localizacao` não traz impacto significativo para o modelo de regressão. Assim, pelo princípio da parcimônia, optamos por selecionar o modelo mais simples, sem `Localizacao,` o m5.
